...
public class ChatService {
    ...
    public static void main(String[] args) throws Exception {
        ...
        ExecutorService executor = Executors.newFixedThreadPool(Math.min(Runtime.getRuntime().availableProcessors(), 8));
        ...
        while (true) {
            ...
            executor.submit(() -> handleClient(client, executor));
        }
    }

    static void handleClient(Socket client, ExecutorService executor) {
        try {
            ...
            while (true) {
                ...
                if (msgType == TYPE_CALL_REQUEST) {
                    ...
                    executor.submit(() -> {
                        try {
                            handleCall(p, rid, out, writeLock);
                        } catch (Exception e) {
                            // Client disconnected
                        }
                    });
    ...
    static void handleCall(byte[] payload, int requestId, OutputStream out, ReentrantLock writeLock) throws Exception {
