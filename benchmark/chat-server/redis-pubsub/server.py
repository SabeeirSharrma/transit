#!/usr/bin/env python3
"""Redis Pub/Sub server for chat server benchmark."""
...
class RedisPubSubServer:
    """Redis Pub/Sub server implementing chat operations."""
    ...
    def start(self):
        """Start the server."""
        self.pubsub.subscribe(**{'benchmark.request': self._handle_request})
        
        print("Redis Pub/Sub server started")
    ...
    def _handle_request(self, message):
        ...
        try:
            ...
            self.redis_client.publish('benchmark.response', json.dumps(response))
            ...
            self.redis_client.publish('benchmark.response', json.dumps(error_response))
...
def serve():
    """Start the Redis Pub/Sub server."""
    server = RedisPubSubServer()
    server.start()
