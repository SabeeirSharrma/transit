...
class TransitServer:
    ...
    def _handle_call(self, payload, request_id, client):
        try:
            ...
            if fn is None:
                status = STATUS_ERROR
                result_json = json.dumps({"error": f"Function '{fn_name}' not found"})
            else:
                try:
            ...
            resp = struct.pack(HEADER_FORMAT, PROTOCOL_VERSION, TYPE_CALL_RESPONSE, request_id, 1 + 4 + len(result_bytes))
