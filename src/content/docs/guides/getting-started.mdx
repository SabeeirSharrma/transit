...
| **C/C++** compiler | For C/C++ functions (GCC, Clang, or MSVC) | `gcc --version` or `g++ --version` |
...
If you only want to use Rust (no Java, Python, or C/C++), you only need Node.js, bun, and Rust.
...
`transit init` scans your source files for `transit.rust()`, `transit.java()`, `transit.python()`, `transit.c()`, and `transit.cpp()` calls, detects which languages you are using, and creates `transit.config.json`. It is idempotent — safe to run multiple times.
...
| **C** | Functions matching generated `transit_c_glue.gen.h` signatures |
| **C++** | Functions matching generated `transit_cpp_glue.gen.h` signatures |
