...
### `transit.python(dir, options?): FunctionProxy`
...
:::caution[Python functions receive JSON strings]
Your Python functions receive a JSON **string**, not a dictionary. Always use `json.loads(args)` to parse it.
:::
...
### `transit.c(directory): FunctionProxy`

Scans a C source directory and returns a language handle.
...
**Parameters:**
- `dir` (string) — path to the directory containing C source files

**Returns:** A `FunctionProxy` — same interface as Rust/Java.

**Behavior:**
- Scans for C source files
- Detects exported functions matching generated `transit_c_glue.gen.h` signatures
- On first function call, loads the compiled native addon (`.node` or `.so`) from the directory
- Caches the handle — calling `transit.c(dir)` again with the same path returns the same handle

**Transport:** In-process native addon. Zero serialization overhead.
...
### `transit.cpp(directory): FunctionProxy`
...
**Behavior:**
- Scans for C++ source files
- Detects exported functions matching generated `transit_cpp_glue.gen.h` signatures
- On first function call, loads the compiled native addon (`.node` or `.so`) from the directory
- Caches the handle — calling `transit.cpp(dir)` again with the same path returns the same handle
