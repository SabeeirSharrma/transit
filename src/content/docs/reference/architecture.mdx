...
│   await c.processChunk(data)                                  │
│   await cpp.fastCompute(n)                                    │
└─────────────┬────────────────────────────────┬───────────────┘
              │                                │
              ▼                                ▼
┌─────────────────────┐          ┌─────────────────────────────┐
...
│  Shared for Rust, C, and C++    │    └──────────────┬──────────────────┘
└──────────────────┬───────────────┘                   │
                   │                                   │
                   ▼                                   ▼
┌──────────────────────────────────┐    ┌─────────────────────────────────┐
│       Your Rust/C/C++ Codebase   │    │      TransitServer (Java)       │
...
- `RustDevBridge` — loads the native addon, dispatches calls (shared for Rust, C, and C++)
...
**Commands:**
- `transit init` — Scans source files for `transit.rust()`, `transit.java()`, `transit.python()`, `transit.c()`, and `transit.cpp()` calls, detects languages, and writes `transit.config.json`
- `transit dev` — Live development mode with file watching, automatic re-scanning, and function change logging
- `transit build` — Build pipeline that orchestrates scan → codegen → compile. Generates typed stubs and compiles Rust/Java/C/C++ code
...
| JS ↔ C | In-process native addon | N/A (direct memory) | ~ns |
| JS ↔ C++ | In-process native addon | N/A (direct memory) | ~ns |
...
V8 (JavaScript) and native Rust/C/C++ code can share a process via N-API — this is how Node.js native addons work. A Rust function compiled with napi-rs is loaded directly into the Node.js process, giving true zero-copy interop. C and C++ use the same mechanism through `binding.gyp` or `CMakeLists.txt` builds.
...
- C/C++: loads the `.node` addon directly (same mechanism as Rust)
...
- C/C++: `node-gyp` or `cmake-js` compiles the native addon
...
**C/C++ errors:** Same as Rust — N-API propagates errors as JavaScript exceptions. The error message from C/C++ is included in the exception.
