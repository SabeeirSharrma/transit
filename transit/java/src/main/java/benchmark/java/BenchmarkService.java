...
public class BenchmarkService {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (running.get()) {
            try {
                ...
                exec.submit(() -> handleClient(client, svc, running));
            } catch (IOException e) {
    ...
    private static void handleClient(Socket client, BenchmarkService svc, AtomicBoolean running) {
        try (client) {
            ...
            while (running.get() && !client.isClosed()) {
                ...
                if (type == TYPE_CALL_REQUEST) {
                    ...
                    resp.put(PROTOCOL_VERSION).put(TYPE_CALL_RESPONSE).putInt(requestId)
                        .putInt(1 + 4 + rb.length).put(status).putInt(rb.length).put(rb);
                    out.write(resp.array()); out.flush();
